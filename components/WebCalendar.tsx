...
const WebCalendar = ({ handleTaskClick}: WebCalendarProps) => {
    ...
    const daySize = (((containerWidth - 78) / 7));
    ...
    React.useEffect(() => {
      ...
      const unsubscribe = onSnapshot(tasksQuery, (tasksSnapshot) => {
        ...
        if (selectedDay !== null) {
          setTasksForSelectedDay(tasksGroupedByDay[selectedDay] || []);
        }
      });
  ...
  return (
    <View style={styles.container}>
      ...
      <View>
        <View style={styles.calendarGrid}  onLayout={handleLayout}>
            {days.map((day, index) => {
            if (day === null) {
      ...
      <View>
        {tasksForSelectedDay.length > 0 ? (
          tasksForSelectedDay.map((task, index) => (
            <TouchableOpacity 
              key={task.id} 
              style={[styles.taskContainer, {zIndex: 9999}]}
            ...
            >
              ...
              <View style={{ flex: 1, paddingHorizontal: 16 }}>
                ...
                <ThemedText style={{ fontSize: 12 }}>Priority: {task.priorityLevel}</ThemedText>
              </View>
            </TouchableOpacity>
          ))
        ) : (
