...
const home = () => {
  ...
  React.useEffect(() => {
    ...
    const unsubscribe = onSnapshot(userDocRef, (userDocSnap: { exists: () => any; data: () => any; }) => {
  ...
  return (
    ...
    <View style={styles.row}>
      <View style={{paddingBottom: 28}}>
        <ThemedText type='big'>Hello, {userName}</ThemedText>
        <ThemedText type='default'>Ready to do some work?</ThemedText>
      </View>
    </View>
    ...
    <View style={{gap: 8}}>
      ...
      {todayTasks.length > 0 ? (
        todayTasks.map((task) => (
          <TouchableOpacity key={task.id} style={styles.taskItem} onPress={() => router.push({ pathname: "/EditTask", params: { taskId: task.id } })}>
            <Checkbox
    ...
    <CustomNavbar selectedIcon='home'/>
  </View>
  );
};
